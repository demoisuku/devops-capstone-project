...
def create_accounts():
    ...
    location_url = url_for("read_account", account_id=account.id, _external=True)  # Remove once get_accounts has been implemented
...
def update_accounts(account_id):
    ...
    account.deserialize(request.get_json())  # keep id stable
    account.id = account_id
    # starter model typically has update(); if yours uses save(), swap accordingly:
    account.update()
    return jsonify(account.serialize()), status.HTTP_200_OK
...
def delete_accounts(account_id):
    ...
    account = Account.find(account_id)
    if not account:
        # if your tests expect idempotent delete, return 204 here instead of 404
        abort(status.HTTP_404_NOT_FOUND, f"Account with id '{account_id}' was not found.")
    account.delete()
